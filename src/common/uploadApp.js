...
class RNUpdate extends Component {
	...
	static defaultProps = {
		...
		bannerImage: require('./imgs/banner.png'),
		closeImage: require('./imgs/close.png'),
	}
	...
	checkUpdate = async () => {
		const fetchRes = await this.props.onBeforeStart()
		console.warn('fetchRes=', fetchRes)
		try {
...
//					})
	...
	androidUpdate = async () => {
		...
		this.filePath = `${RNFS.ExternalDirectoryPath}/${filename}`
		
		console.warn('下载包的路径', this.filePath)
		// 检查包是否已经下载过，如果有，则直接安装
		let exist = await RNFS.exists(this.filePath)
		if (exist) {
			console.warn('下载包存在了', exist)
			try {
